...
class NumpyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input = np.zeros([1, 1, 512, 512]).astype(np.float32)
        input[0, 0] = cv2.resize(data, (512, 512))
        label = np.zeros([1, 1, 256, 256]).astype(np.float32)
        label[0, 0] = data
        return input, label
...
class Loss(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.norm = nn.LayerNorm([256, 256], elementwise_affine=False)
        
    def forward(self, reconstruction, label):
        target = reconstruction[:, :, (256-128):(256+128), (256-128):(256+128)]
...
if __name__ == '__main__':
    ...
    trainset = NumpyDataset('/data/gzy/anime/anime_val.pkl')
